...
MANDATORY_SCANS = [
    ...
    {"id": "vuln", "name": "Vulnerability and Exploit Scan", "command": "nmap -Pn --script vuln,exploit --script-args http-slowloris.runforever=false,http-slowloris.timelimit=60s {ip}"},
...
# Per-scan timeout overrides (seconds). Falls back to DEFAULT_SCAN_TIMEOUT.
SCAN_TIMEOUTS = {
    "ssl":        600,   # ssl-enum-ciphers is usually fast
    "cert":       600,
    "headers":    600,
    "methods":    600,
}
...
class ScanJob:
    ...
    timeout: int = field(compare=False, default=DEFAULT_SCAN_TIMEOUT)
...
def execute_single_scan(job: ScanJob, logger: logging.Logger) -> Dict[str, Any]:
    ...
    try:
        ...
        output_file.write_text(f"ERROR: Scan timed out after {timeout}s\n")
        logger.error(f"Scan {job.scan_id} on {target_name}: TIMEOUT after {timeout}s")
        return {"scan_id": job.scan_id, "success": False, "duration": timeout, "status": "timeout"}
    except Exception as e:
...
def build_mandatory_jobs(target: Dict[str, str], output_dir: Path,
    ...
    for scan in MANDATORY_SCANS:
        ...
        timeout = SCAN_TIMEOUTS.get(scan_id, DEFAULT_SCAN_TIMEOUT)
        jobs.append(ScanJob(
            priority=priority,
            scan_id=scan_id,
...
class ScanProgress:
    ...
    def complete(self, key: str, success: bool, duration: int, status: str = ""):
        with self._lock:
            if key in self._tasks:
                mins = duration // 60
                secs = duration % 60
                ...
                if status == "timeout":
                    self._tasks[key]["status"] = "TIMEOUT ✗"
                else:
                    self._tasks[key]["status"] = "DONE ✓" if success else "FAILED ✗"
            ...
            for future in done:
                ...
                try:
                    ...
                    if scan_result["success"]:
                        ...
                        progress.complete(key, True, scan_result["duration"], scan_status)
                    else:
                        result["scans_failed"].append(job.scan_id)
                        result["skip_reasons"][job.scan_id] = scan_status
                        progress.complete(key, False, scan_result["duration"], scan_status)
                except Exception as e:
                    result["scans_failed"].append(job.scan_id)
                    result["skip_reasons"][job.scan_id] = str(e)
                    progress.complete(key, False, 0, "error")
