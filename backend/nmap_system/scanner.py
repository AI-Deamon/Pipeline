...
MANDATORY_SCANS = [
    {
        "id": "ssl",
        "name": "SSL Cipher Enumeration",
        "command": "nmap -sV --script ssl-enum-ciphers {ip}",
    },
    {
        "id": "vuln",
        "name": "Vulnerability and Exploit Scan",
        "command": "nmap -Pn --script vuln,exploit {ip}",
    },
    {
        "id": "slowloris",
        "name": "HTTP Slowloris Check",
        "command": "nmap --max-parallelism 750 -Pn --script http-slowloris {ip}",
    },
...
AGGRESSIVE_SCANS = [
    {
        ...
        "command": "nmap -Pn --script brute {ip}",
    },
    {
        "id": "dos",
        "name": "DoS Vulnerability Scan",
        "command": "nmap --script dos -Pn {ip}",
    },
]
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    for target in config["targets"]:
        ...
        if target["environment"] not in valid_environments:
            raise ValueError(
                f"Invalid environment for {target['name']}: {target['environment']}"
            )
...
def get_scan_header(
    scan_name: str, target: Dict[str, str], command: str, date_str: str, time_str: str
) -> str:
...
def parse_discovery_output(raw_output: str) -> List[Dict[str, Any]]:
    ...
    open_ports = []
    port_pattern = re.compile(r"^(\d+)/(\w+)\s+open\s+(\S+)(?:\s+(.*))?$", re.MULTILINE)
...
def run_discovery_scan(
    ...
    try:
        ...
        result = subprocess.run(
            command_list, capture_output=True, text=True, timeout=3600
        )
        ...
        unknown_ports = [
            p["port"]
            for p in open_ports
            if not p["service"] or p["service"] == "unknown"
        ]
...
def execute_single_scan(job: ScanJob, logger: logging.Logger) -> Dict[str, Any]:
    ...
    # Limit each host scan to 10 minutes to prevent CI pipeline timeouts
    if "--host-timeout" not in command_str:
        command_str += " --host-timeout 10m"
    ...
    try:
        ...
        logger.info(
            f"Scan {job.scan_id} on {target_name}: {'success' if success else 'failed'} ({duration}s)"
        )
...
def determine_service_scans(
    ...
    for scan_id, scan_info in SERVICE_SCAN_MAP.items():
        ...
        if matched:
            ...
            scan_data["ports"] = (
                ",".join(matched_ports) if matched_ports else triggers[0]
            )
...
def build_mandatory_jobs(
    target: Dict[str, str], output_dir: Path, scan_filter: Optional[str] = None
) -> List[ScanJob]:
...
class ScanProgress:
        ...
        with self._lock:
            if key in self._tasks:
                ...
                self._tasks[key]["status"] = (
                    "DONE ✓"
                    if success
                    else f"{'TIMEOUT' if duration >= 3600 else 'FAILED ✗'}"
                )
    ...
    def done_count(self) -> int:
        with self._lock:
            return sum(
                1 for t in self._tasks.values() if t["status"].startswith("DONE")
            )
    ...
    def failed_count(self) -> int:
        with self._lock:
            return sum(
                ...
                if "FAILED" in t["status"] or "TIMEOUT" in t["status"]
            )
...
def build_progress_table(progress: ScanProgress, total_jobs: int) -> Table:
    ...
    for task in tasks:
        ...
        table.add_row(
            task["host"], task["scan_name"], f"{status_style}{status}[/]", duration
        )
            ...
            for future in done:
                ...
                try:
                    ...
                    if scan_result["success"]:
                        ...
                        result["skip_reasons"][job.scan_id] = scan_result.get(
                            "status", "unknown"
                        )
...
def run_scanner(
    ...
    if target_filter:
        ...
        if not targets:
            console.print(
                f"[bold red]Error:[/bold red] Target '{target_filter}' not found"
            )
            sys.exit(1)

    # Thread-safe summary tracker
    summary_lock = threading.Lock()
    scan_summary = {"date": date_str, "client": config["client"], "targets": {}}
    ...
    if dry_run:
        for target_idx, target in enumerate(targets, 1):
            ...
            console.print(
                f"  [yellow]Service scans: (depends on discovery results)[/yellow]"
            )
            for scan_id, scan_info in SERVICE_SCAN_MAP.items():
                if scan_filter and scan_id != scan_filter:
                    continue
        ...
        with ThreadPoolExecutor(max_workers=num_discovery_workers) as executor:
            ...
            for target in targets:
                future = executor.submit(
                    run_discovery_scan, target, raw_dir, logger, False
                )
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    console.print(
                        f"  [green]✓[/green] {target['name']}: {len(open_ports)} open ports"
                    )
    ...
    with Live(
        build_progress_table(progress, 0), refresh_per_second=2, console=console
    ) as live:
        with ThreadPoolExecutor(max_workers=num_discovery_workers) as executor:
            futures = {}
            for target in targets:
        ...
        for target in targets:
            result = discovery_results.get(
                target["name"], {"open_ports": [], "unknown_ports": []}
            )
            handle_unknown_services(target, result, raw_dir, logger, False)
    ...
    for target in targets:
        target_summary = {
            ...
            "environment": target["environment"],
            "discovery": {
                "open_ports": discovery_results.get(target["name"], {}).get(
                    "open_ports", []
                ),
                "unknown_ports": discovery_results.get(target["name"], {}).get(
                    "unknown_ports", []
                ),
            },
            "scans_run": [],
    ...
    console.print(
        f"  [bold]{runnable_jobs}/{len(all_jobs)} jobs will execute, {num_workers} workers, priority-ordered[/bold]\n"
    )

    # Run parallel executor with live progress table
    try:
        with Live(
            build_progress_table(progress, len(all_jobs)),
            refresh_per_second=1,
            console=console,
            transient=False,
        ) as live:
            result = run_parallel_executor(
                all_jobs, progress, num_workers, logger, live_display=live
            )
            # Final table refresh
...
Examples:
  ...
  python3 scanner.py --dry-run                # Show what would run
    ...
    parser.add_argument(
        "--date", type=str, help="Use specific date folder (YYYY-MM-DD)"
    )
    ...
    if not config_file.exists():
        ...
        console.print(
            "Expected location: pentest_system/targets.json or current directory"
        )
        sys.exit(1)
