...
""")
    ...
    def _get_previous_scan_severity(self) -> dict:
        """Aggregate severity_summary from ALL reports of the previous completed scan.

        Queries by ScanDB.finished_at to get the correct previous scan,
        then sums severity counts across all tools for that scan.
        """
        ...
        db = SessionLocal()
        try:
            # Step 1: Find the previous completed scan (by finished_at)
            previous_scan = (
                db.query(ScanDB)
            ...
            # Step 2: Aggregate ALL reports for that scan
            reports = (
