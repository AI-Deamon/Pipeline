...
def verify_callback_token(x_callback_token: str = Header(...)) -> None:
    if not hmac.compare_digest(x_callback_token, settings.CALLBACK_TOKEN):
        raise HTTPException(status_code=403, detail="Invalid callback token")


@router.get("/scanner.py",
  responses={403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def get_scanner_script(x_callback_token: Annotated[str, Header(alias="X-Callback-Token")] = ...):
...
@router.get("/parser.py",
  responses={403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def get_parser_script(x_callback_token: Annotated[str, Header(alias="X-Callback-Token")] = ...):
    verify_callback_token(x_callback_token)
